...
可选字符串字段**为空时要省略 key**,绝不能发 JSON `null`(Android `org.json` 的 `optString` 会把 null 读成字符串 `"null"`)。`putIfNonEmpty` 帮你做这件事。这是 [协议保真原则](/contributing/server/protocol-fidelity) 的硬约束。
:::
...
读一个现成的简单端点(比如 `client` 包的 `methodSelect`)作模板即可。下一步看 [代码规范](/contributing/server/conventions)。
