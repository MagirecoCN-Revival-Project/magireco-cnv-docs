...
测试默认用 SQLite 内存库;涉及方言差异(UPSERT、RETURNING、JSON)的改动,**最好也在 PG/MySQL 上验一遍**(Docker 起一个即可,见 [开发环境搭建](/contributing/server/dev-setup#用-postgresqlmysql-开发-可选))。`store_test.go` 的烟雾测试已经覆盖了 RETURNING/LastInsertId 两条路径。
