...
3. **三种数据库** —— 写 SQL 用 `?` 占位符,经存储层方言适配 PG/MySQL/SQLite。别写某一种数据库专属的语法。详见 [存储层与多方言](/contributing/server/store-dialects)。
4. **协议保真** —— `/client/*` 的字段名、嵌套、空值处理对齐客户端 Java 源码,有保真测试守护。改这些前必读 [协议保真原则](/contributing/server/protocol-fidelity)。
...
1. [开发环境搭建](/contributing/server/dev-setup) — 把项目在本地跑起来。
2. [代码库导览](/contributing/server/codebase-tour) — 知道每个目录在干什么。
3. [运行与编写测试](/contributing/server/testing) — 学会跑测试、写测试。
4. [新增一个接口](/contributing/server/adding-endpoint) — 跟着做一遍完整流程。
5. [代码规范](/contributing/server/conventions) — 提交前对照。
