...
`/client/*` 的字段名、嵌套、空值处理对齐客户端 Java 源码,**不是你说改就能改**:
...
详见 [协议保真原则](/contributing/server/protocol-fidelity)。这是最容易出事、也最该谨慎的地方。
...
详见 [多方言抽象](/contributing/server/store-dialects)。
...
- **直接提交 `main`**:本仓库不走功能分支 / PR 流程。改 `/client/*`、`/account/*` 契约的代码必须**同提交**带上文档,详见 [契约纪律与文档绑定](/contributing/server/discipline)。
