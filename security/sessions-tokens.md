...
两个服务端**都实现了同一套签发与校验**(`internal/clienttoken`),但在部署里承担的角色不同:

| | 签发 | 校验 |
|---|---|---|
| API 服务端 | 是,**身份的源头** | 自己签的 |
| 资源分发服务端 | 独立部署时自签自认 | 接上 API 服务端后,凭公钥认它签的 |

接线只有一步:把 API 服务端启动日志里那行「客户端会话令牌验证公钥」的值,填进资源分发服务端的 `CNV_CLIENT_TOKEN_TRUSTED_KEYS`。**只传公钥**——这正是非对称签名换来的东西,资源分发服务端拿到它只能验、不能签。

::: danger 会话令牌签名种子必须在线,而且它就是身份本身
`CNV_CLIENT_TOKEN_SEED` 泄漏 = 攻击者能凭空造出任意 `device_id`、任意账号的会话,而且**下游根本查不出来**——那边不查库,验签通过就是通过。

它与离线的[根 CA 私钥](/security/node-pki)、目录私钥是三把**不同**的钥匙,任何一把都不能拿来兼任另一把的职责。
:::
