...
| 术语 | 含义 |
|---|---|
| **在线下载** | *(APK 时代)* 多镜像并发分片下载游戏资源 |
| **资产分发面 / resource plane** | 现役方案:边缘节点用 Bearer 令牌鉴权,提供 S3 风格清单与支持 Range 的文件下载 |
| **`asset_auth`** | `/client/init` 下发的短时资产凭证信封 `{type, token, expires_at}`;与 `access_token` 是两把不同的钥匙 |
| **边缘节点 / edge** | 只有 `resource` 能力的节点,只发资产、拿不到凭证类请求 |
| **离线包注入** | *(APK 时代)* 用本地 zip 两阶段解压准备资源 |
| **临时离线注入 / provisional** | 服务器不可达时的应急注入，仅本地校验、不算正式安装 |
| **断点续传** | 中断后从断点继续，依赖 HTTP Range |
| **`.cnvprog`** | 分片下载的断点元数据文件 |
| **镜像 / mirror** | *(APK 时代)* 同一资源的多个 CDN 节点。现由边缘 resource 节点取代 |
| **线路 / group** | *(APK 时代)* 一组镜像 |
| **换线 / switch_mirrors** | *(已删除)* 心跳指挥客户端切换镜像 |
| **热更新 / hot-update** | *(已删除)* js / scenario 包的增量更新 |
