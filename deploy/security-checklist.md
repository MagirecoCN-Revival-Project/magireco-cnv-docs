...
- [ ] **资产令牌密钥已配置且各节点一致**
  业务节点首次启动自动生成并持久化,默认无需管理;**边缘节点必须显式配
  `CNV_RESOURCE_TOKEN_SECRET`**(≥16 字节)且与业务节点相同,否则所有资产请求 401。
  `CNV_RESOURCE_TOKEN_WINDOW_SEC` 同理,两边必须同值。
- [ ] **资产目录不是裸挂的**
  `/res` 下每个请求都要过 `Authorization: Bearer`。若你在反代层另开了一条绕过节点的
  静态文件路径,那条路径就没有鉴权——等于把整棵资产树公开。
...
- [ ] **数据库定期备份**
  至少覆盖 `config` / `accounts` / `saves`。见 [选择数据库](/deploy/database#备份建议)。
