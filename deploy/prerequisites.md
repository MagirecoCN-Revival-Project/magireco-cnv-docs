...
如果你想在多个地区放下载节点,准备额外的服务器跑 `magireco-node`(设 `CNV_NODE_ROLE=edge`);用 `magireco-panel`(需 `CNV_PANEL_KEY`,**≥16 字符**)统一注册与监控这些节点。见 **[节点与面板](/deploy/nodes)**。
...
准备好了就去 **[快速部署](/deploy/quick-start)**。
