...
3. **游戏数据库** — 配置并测试**业务节点**要连的游戏库（玩家账号 / 云存档）。可选 **SQLite / MySQL / PostgreSQL**，填好连接参数后做一次真实连接测试——复用节点同一套 DSN 解析与 `Open`（含 ping），"此处测得通即节点连得上"；向导随即给出对应的 `CNV_DB_URL`，供你设置到业务节点环境变量。见 [选择数据库](/deploy/database)。
...
1. 找到 `magireco-node` 二进制。**默认**：面板自身二进制所在目录（同目录平级）。**覆盖**：设置 [`CNV_NODE_BIN`](/deploy/configuration#cnv_node_bin) 为绝对路径。
