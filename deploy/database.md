...
主节点支持 **PostgreSQL / MySQL / SQLite** 三选一。同一套代码通过**方言抽象**适配三者(技术细节见 [存储层与多方言抽象](/contributing/server/store-dialects))。这里只讲怎么选、连接串怎么写。
...
迁移会创建约 18 张表。完整字段见 **[数据模型](/server/data-model)**,这里给个全貌:
