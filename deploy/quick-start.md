...
迁移会在启动时自动跑，无需手动建表。详见 **[选择数据库](/deploy/database)**。
...
> **生产形态**：起面板后统一打开**面板**地址（默认 `:8090`）登录；面板托管全部前端、浏览器跨域直连业务节点。给业务节点设 `CNV_PANEL_PUBLIC_URL=<面板URL>` 打通入口跳转与 CORS，详见 [节点与面板](/deploy/nodes#面板托管前端与跨域直连)。
...
- 在自身二进制平级目录找 `magireco-node`（[`CNV_NODE_BIN`](/deploy/configuration#cnv_node_bin) 可覆盖）；
...
- ⚠️ **上生产前**务必过一遍 **[安全加固清单](/deploy/security-checklist)**（签名白名单、trust proxy、TLS…）。
- 把进程托管成 systemd 服务、配好日志与备份，见 **[日常运维](/deploy/operations)**;
...
- 配置反代与多域名，见 **[反向代理与域名](/deploy/reverse-proxy)**。
