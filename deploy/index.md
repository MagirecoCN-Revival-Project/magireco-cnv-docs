...
这是「魔法纪录复兴计划」的**服务端**。如果你想为本计划的**客户端**(一个安卓 APK)搭一个自己的后端,你来对地方了。
...
然后用 `admintool` 建第一个管理员,登录后台即可。完整步骤见 **[快速部署](/deploy/quick-start)**。

## 接下来读什么

1. **[部署前准备](/deploy/prerequisites)** — 你需要先有什么(服务器、域名、数据库、客户端签名)
2. **[快速部署](/deploy/quick-start)** — 一步步把主节点跑起来
3. **[选择数据库](/deploy/database)** — 三种数据库怎么选、连接串怎么写
4. **[环境变量参考](/deploy/configuration)** — 所有 `CNV_*` 配置项
5. **[安全加固清单](/deploy/security-checklist)** — 上生产前**务必**过一遍
